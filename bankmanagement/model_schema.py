...
class Customer(db.Model):
	...
	def __init__(self, name, username, password,address,state,country,email,pan,dob,accountType,contactNo):
		self.name = name
...
class CustomerSchema(ma.SQLAlchemySchema):
	...
	pan = fields.Integer(required = True, validate = Range(min=10))
	...
	contactNo = fields.Integer(required = True,validate = Range(min=10))

	class Meta:
		fields = ("id", "name", "password","username","address", "state", "country","email","pan","dob","accountType","contactNo")
...
class Loan(db.Model):
    ...
    def __init__(self, loanType, loanAmount,date,interest,duration,customer_id):
...
class LoanSchema(ma.SQLAlchemyAutoSchema):
	...
	class Meta:
		fields = ("id", "loanType", "loanAmount","date", "interest", "duration","customer_id")
